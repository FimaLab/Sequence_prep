...
def fill_sp_table(_samples_list, _frozen_table_file=False):
    #this func iterates frozen_samlples cells in columns values. check if it not frozen then write sample into that cell. returns dataframe filled sample preparation table
    def iterator(_df):
        try:
...
def draw_sp_image(df):
    #график из таблицы подготовки образцов
    # Настройка графика
...
if __name__ == "__main__":
    ...
    with c2:
        with st.container(border=True):
            ...
            with c21:
                st.write('Preview')
                st.write('First randomization', key='preview')
                if 'ran' in st.session_state:                    
                    st.write(first, key='preview')                    
            with c22:
                st.write('Batches') 
                st.write("")
                if 'ran' in st.session_state:
        ...
        with st.container(border=True):
            ...
            with c32:
                ...
                if 'start' in st.session_state:
                    if 'ran' in st.session_state:
                        for i, batch in enumerate(batches, 1):                        
                            #batch list of samples, batch_n_group - df, real - df with numbers as index, real_position_group - df with sample name as index posiotin n group ascolumns
                            #add group to batch samples
                            batch_n_group = sample_n_group.loc[batch]
                            #fill ideal_sequence prior second randomization
                            real = ideal_in2real(batch_n_group)
                            #link sapmle position to sample name
                            real_position_group = real.loc[real['Sample name'].isin(batch)][['Sample name','Sample Position', 'Sample Group']].set_index('Sample name')                            
                            #second randomization keeping sample position tied
                            batch_n_group_pos_rand = randomize_samples_first(real_position_group)
                            #fill ideal_sequence after second randomization
